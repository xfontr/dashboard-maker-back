...
describe("Given a validationError middleware", () => {
  ...
  describe("When called with a joi error", () => {
    ...
    test(`Then it should respond with a status of '${HTTP_CODES.error.badRequest}' and a error message`, () => {
      validationError(joiError, req, res as Response, next);

      expect(res.status).toHaveBeenCalledWith(HTTP_CODES.error.badRequest);
